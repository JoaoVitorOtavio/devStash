...
- 2026-08-01: Completed Actions Duplication Refactor:
  ...
  - Extracted `getOwnedCollectionIds`, `syncItemTags`, `syncItemCollections` helpers in `src/server/db/items.ts`, used by both `createItem` and `updateItem` instead of duplicating the tag find-or-create loop and collection-linking loop in each.
  - Collapsed `toggleItemFavorite`/`toggleItemPin` (`src/actions/items.ts`) into a single generic `toggleItemFlag(itemId, field)`, since both were identical aside from the field name.
  - Found and fixed a real build break introduced mid-refactor: standardizing `updateItem`'s success flag to `Boolean(true)`/`Boolean(false)` (per the project's `Boolean()` convention for new/edited actions) widened its return type to plain `boolean`, breaking discriminated-union narrowing on `result.data` in `item-drawer.tsx` (which spreads `...result.data` after `if (!result.success) return`). `createItem`/`deleteItem` callers never touch `.data`, so `Boolean()` is safe there; `updateItem` specifically needs `true as const`/`false as const` for its caller to type-check, so it was kept on `as const` intentionally, not accidentally inconsistent.
  - Scan performed via the `refactor-scanner` subagent across `src/actions/*.ts`, `src/server/db/items.ts`, and two API routes; only the 4 HIGH-impact findings were fixed (auth boilerplate, schema duplication, tag/collection sync duplication, toggle duplication). MODERATE/OPTIONAL findings (bcrypt salt-round helper, password-match check, missing try/catch per `coding-standards.md`, API-route auth duplication) were left for a future pass.
  - `npm run build` clean, `npm test` (98 tests) passing unchanged.
...
- 2026-07-29: Completed Homepage:
  ...
  - Only 4 Client Components, matching the spec's split: `Navbar` (scroll-opacity), `ChaosVisual` (requestAnimationFrame drift/bounce/mouse-repel, ported from the prototype), `ScrollReveal` (generic IntersectionObserver fade-in wrapper reused by every section), and `PricingToggle` (monthly/yearly, owns both pricing cards since the Pro price is the only dynamic bit). Everything else is a Server Component.
  - Caught and fixed a real inaccuracy in the spec itself before implementing: the item-type accent colors/icons the spec had copied from the mockup didn't match the actual seeded values in `prisma/seed.ts` (e.g. real Prompt is `#8b5cf6`/`Sparkles`, not the mockup's `#f59e0b`; real Command is `#f97316`/`Terminal`, not `#06b6d4`). Corrected the spec and `item-type-palette.ts` to the real seed values so the homepage never drifts from what the app actually shows.
  - All links wired to real routes: Sign In → `/sign-in`, Get Started/Get Started Free/Upgrade to Pro → `/register`, nav Features/Pricing → `#features`/`#pricing` anchors — verified via a DOM query of every anchor's href in the browser.
  - Unit tests unaffected (98 passing, this feature added no server/DB code); `npm run build` clean, `/` now shows as a static route.
  - Verified end-to-end in the browser via Playwright MCP: hero/features/AI/pricing/CTA/footer all render and reveal on real scroll, pricing toggle switches $8/mo ↔ $72/yr, no console errors, desktop (1440px) and mobile (390px) both checked.
  - Found and fixed two real bugs during that verification:
    - Mobile (390px) had genuine horizontal page overflow (documentElement.scrollWidth 443 vs innerWidth 390) from a classic CSS Grid blowout: the AI section's `<pre>` code block has intrinsic `white-space: pre` width, and a grid item's default `min-width: auto` let that width force the `md:grid-cols-2` track wider than its column, extending past the viewport. Fixed with `min-w-0` on the grid item and its inner code-window wrapper so `overflow-x-auto` on the `pre` can actually do its job.
    - `ChaosVisual` measured its container's width/height once in a mount-only `useEffect` and reused that forever, so resizing the viewport after mount (e.g. dev tools/testing, or a user resizing their browser) left particles bouncing against walls sized for the old viewport — on mobile this let an icon's absolute position exceed the now-narrower container. Fixed by re-measuring `getBoundingClientRect()` every animation frame instead of once, so the walls always match the current layout.
