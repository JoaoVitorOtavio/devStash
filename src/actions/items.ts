...
async function toggleItemFlag(itemId: string, field: "isFavorite" | "isPinned") {
  const { userId, error } = await requireUserId();
  if (!userId) return { success: Boolean(false), error };

  const item = await prisma.item.findUnique({ where: { id: itemId, userId } });
  if (!item) return { success: Boolean(false), error: "Item not found" };
  ...
  return { success: Boolean(true), data: { [field]: updated[field] } };
}

export async function toggleItemFavorite(itemId: string) {
  return toggleItemFlag(itemId, "isFavorite");
}

export async function toggleItemPin(itemId: string) {
  return toggleItemFlag(itemId, "isPinned");
}
...
export async function createItem(payload: CreateItemPayload) {
  ...
  const created = await createItemQuery(userId, { typeName: type, ...rest });
